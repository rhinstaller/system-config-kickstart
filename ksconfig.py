...
list_items = [ "Czech", "English", "French", "German", "Hungarian", "Icelandic", 
			"Italian","Japanese", "Norwegian", "Romanian", "Russian", "Serbian", "Slovak",
...
#---------------Language Support----------------#
languagesupportLabel = GtkLabel("Language Support:")
table1.attach(languagesupportLabel, 0, 1, 3, 4)

languagesupportCombo = GtkCombo()
table1.attach(languagesupportCombo, 1, 2, 3, 4)

languagesupportCombo.set_popdown_strings(list_items)
languagesupportCombo.list.select_item(1)
languagesupportCombo.entry.set_editable(FALSE)

#---------------Keyboard------------------#
keyboardLabel = GtkLabel("Keyboard:")
table1.attach(keyboardLabel, 0, 1, 4, 5)

keyboardCombo = GtkCombo()
table1.attach(keyboardCombo, 1, 2, 4, 5)
...
table1.attach(mouseLabel, 0, 1, 5, 6)

mouseCombo = GtkCombo()
table1.attach(mouseCombo, 1, 2, 5, 6)
...
table1.attach(timeLabel, 0, 1, 6, 7)

timeCombo = GtkCombo()
table1.attach(timeCombo, 1, 2, 6, 7)
...
table1.attach(rootLabel, 0, 1, 7, 8)

rootEntry = GtkEntry()
rootEntry.set_visibility(FALSE)
table1.attach(rootEntry, 1, 2, 7, 8)
...
vbox1.pack_start(frame4, TRUE)

partVbox = GtkVBox()
frame4.add(partVbox)

mbrHbox = GtkHBox()
partVbox.pack_start(mbrHbox, FALSE)
...
clearHbox = GtkHBox()
partVbox.pack_start(clearHbox, FALSE)
...
partitionScroll = GtkScrolledWindow ()
partitionScroll.set_policy (POLICY_AUTOMATIC, POLICY_AUTOMATIC)
...
partitionScroll.add(partClist)
partVbox.pack_start(partitionScroll, TRUE)
...
partHbox = GtkHBox()
partVbox.pack_start(partHbox, FALSE)
...
vbox1.pack_start(optionsFrame, FALSE)
